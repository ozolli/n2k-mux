...
"  '</td><td>'+esc(s.model||'')+'</td><td>'+(s.pgns||[]).map(p=>'<span class=pill>'+p.pgn+' ×'+p.count+'</span>').join('')+'</td></tr>';}\n"
...
" for(const p of d.pgns||[])h+='<tr><td>'+p.pgn+'</td><td class=n>'+ms(p.hz)+'</td><td class=n>'+p.total+'</td></tr>';\n"
...
" for(const t of d.out_types||[])h+='<tr><td>'+esc(t.type)+'</td><td class=n>'+ms(t.hz)+'</td><td class=n>'+t.total+'</td></tr>';\n"
