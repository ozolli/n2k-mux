...
" let h='<table><tr><th class=n>src</th><th>Nom</th><th class=c>Ignorer</th><th>Identité</th><th>Modèle</th><th>PGNs publiés</th></tr>';\n"
...
" const m={};for(const p of (d.pgns||[]))m[p.pgn]=p.hz;\n"
...
" for(let i=0;i<ARB.units.length;i++){const c=$('#ld'+i);if(c){const hz=m[ARB.units[i].pgn];c.innerHTML=hz?(H(1,hz)+'<small> Hz</small>'):'';}}\n"
"}catch(e){}}\n"
...
"function drawArb(){const dtk=ARB.talker||'II';let h='<table class=arb><tr><th>PGN</th><th>Mode</th><th class=c>N2K</th><th class=c>Talker</th><th class=c>Phrases 0183</th><th class=c>ms</th><th class=c>Hz</th><th>Sources <small>(gauche = prioritaire)</small></th></tr>';\n"
...
"    +'<br><label class=sl><input type=checkbox data-act=ign data-u='+ui+(u.ign?' checked':'')+'>ignorer</label></td>';\n"
