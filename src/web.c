...
"#arb_msg,#src_msg{margin:.6em 0;padding:.5em .8em;border-radius:6px;display:none}#arb_msg.ok,#src_msg.ok{display:block;background:#12351f;color:#7ee787}\n"
"#arb_msg.err,#src_msg.err{display:block;background:#3a1212;color:#ff7b72;white-space:pre-wrap}\n"
...
"<button data-t='arbitrage'>Arbitrage</button></nav>\n"
...
"<small> &nbsp;case = source retenue · ◀▶ = ordre de priorité</small></div>\n"
...
" let h='<table><tr>'\n"
"  +'<th class=n title=\"Adresse source NMEA 2000 (0-251) ; peut changer aux réallocations d’adresse du bus\">src</th>'\n"
"  +'<th title=\"Nom logique éditable. Le saisir ici rend la source utilisable dans l’onglet Arbitrage\">Nom</th>'\n"
"  +'<th class=c title=\"Ignorer toutes les trames de cette source ([ignore] src)\">Ignorer</th>'\n"
"  +'<th title=\"Identité stable de l’appareil (Model Serial Code ou Unique Number) — survit aux changements d’adresse\">Identité</th>'\n"
"  +'<th title=\"Modèle / fabricant déclaré par l’appareil (PGN 126996 Product Information)\">Modèle</th>'\n"
"  +'<th title=\"PGN réellement émis par cet appareil et leur nombre\">PGNs publiés</th>'\n"
"  +'</tr>';\n"
...
" $('#arb_load').innerHTML='Bus N2K '+bar(d.bus_load_pct)+' '+(d.measured?'(mesuré)':'(estimé)')+' &nbsp; 0183 '+bar(d.out_load_pct)+' &nbsp;<small>'+H(1,d.frames_per_s)+' trames/s · '+H(1,d.out_sent_per_s)+' phrases/s</small>';\n"
...
"  +'<th title=\"Numéro de PGN NMEA 2000 (+ discriminant éventuel) et son rôle\">PGN</th>'\n"
"  +'<th title=\"Mode de sélection : priority (1re source vivante), min (profondeur, sécurité), max (loch), fusion (AIS, dédup MMSI)\">Mode</th>'\n"
"  +'<th class=c title=\"Réémettre ce PGN en NMEA 2000 (vcan0 + YDRAW). Décoché = ce PGN est coupé du flux N2K arbitré\">N2K</th>'\n"
"  +'<th class=c title=\"Talker 0183 pour ce PGN (2 lettres, ex. GP, II, HE). Vide = talker global de la sortie\">Talker</th>'\n"
"  +'<th class=c title=\"Phrases 0183 émises pour ce PGN. Tout coché = défaut ; un sous-ensemble = liste blanche [sentence] ; rien coché = pas de 0183 pour ce PGN\">Phrases 0183</th>'\n"
"  +'<th class=c title=\"Intervalle minimum entre deux phrases de ce PGN, en millisecondes (throttle). Vide ou 0 = pas de limite\">ms</th>'\n"
"  +'<th class=srccol title=\"Sources vues sur le bus pour ce PGN. Cochée = retenue ; ordre de gauche à droite = priorité décroissante ; ◀▶ réordonne\">Sources <small>(gauche = prioritaire)</small></th>'\n"
"  +'<th class=c title=\"Nombre total de messages de ce PGN reçus depuis le démarrage du daemon\">Total reçu</th>'\n"
"  +'<th class=c title=\"Fréquence d’arrivée mesurée du PGN (messages par seconde, toutes sources confondues)\">Hz</th>'\n"
"  +'</tr>';\n"
...
"    +'<br><label class=sl title=\"Ignorer complètement ce PGN : ni arbitrage, ni sortie 0183/N2K ([ignore] pgn)\"><input type=checkbox data-act=ign data-u='+ui+(u.ign?' checked':'')+'>ignorer</label></td>';\n"
