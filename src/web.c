...
"sim_boat:'Bateau',sim_cur:'Courant',sim_wind:'Vent vrai',\n"
...
"sim_rand_lbl:'aléatoire',sim_polar_lbl:'polaire',sim_port:'bâbord',sim_stbd:'tribord'\n"
"},en:{\n"
...
"sim_boat:'Boat',sim_cur:'Current',sim_wind:'True wind',\n"
...
"sim_rand_lbl:'random',sim_polar_lbl:'polar',sim_port:'port',sim_stbd:'starboard'\n"
"}};\n"
...
"  // Angles à l'étrave (TWA, AWA, TWA eau) : lecture marine, 0 à 180° + bord.\n"
"  // Le simulateur et les sorties NMEA gardent 0-360, imposé par les normes.\n"
"  let val=H(SIMUNIT[k]?2:1,st[k]),unit=T(SIMUNIT[k]?'sim_kn':'sim_dir');\n"
"  if(k==='twa'||k==='awa'||k==='twa_w'){const a=((st[k]%360)+360)%360,port=a>180,m=port?360-a:a;\n"
"   val=H(1,m);unit=T('sim_dir')+((m>0.05&&m<179.95)?' '+T(port?'sim_port':'sim_stbd'):'');}\n"
"  h+='<tr><td>'+T('sim_'+k)+'</td><td class=n>'+val\n"
"   +' <small>'+unit+'</small></td>'\n"
...
/* Les SIX réglages du simulateur. TWA, AWA et AWS n'en font pas partie : ils
 * sont calculés à partir de ceux-là et publiés dans le fichier d'état. */
static const char *SIM_KEYS[] = {
    "hdg", "stw", "set", "drift", "twd", "tws"
};
